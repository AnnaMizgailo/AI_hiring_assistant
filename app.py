...
async def google_callback(
    ...
    await handle_google_callback(db, recruiter_id, code)
    return RedirectResponse(url="/jobs?google_calendar=connected", status_code=303)


@app.get("/api/google/status", response_model=Dict[str, Any])
async def google_status(
    recruiter: str = Depends(get_current_recruiter),
    db: AsyncSession = Depends(get_db),
):
    return await get_google_status(db, recruiter)
